...
class SSLContextFactory(ssl.ContextFactory):
    ...
    def __init__(self, variables, routes, opts, certificate_path, key_path=None):
    ...
    def pick_certificate(self, connection):
        ...
        ctx = utils.apply_middlewares(self.opts, middlewares, _pick_certificate)(server_name_indication, connection)
        if ctx is not None:
