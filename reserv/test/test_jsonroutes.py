...
class TestJsonRoutes(unittest.TestCase):
    def test_init(self):
        j = JsonRoutes(os.path.join(os.path.dirname(__file__), "fragments", "jsonroutes", "routes.json"))
        
        self.assertEqual(len(j.route_descriptors), 1)

    def test_replace_route(self):
        j = JsonRoutes(os.path.join(os.path.dirname(__file__), "fragments", "jsonroutes", "routes.json"), variables={"replace" : "XXX"})
